...
export default function MonthlyView({ events, onEventClick, onWeekClick }) {
  ...
  const generateYear2026 = () => {
    const weeks = [];
    let currentDate = new Date(2025, 11, 28);
    let weekNum = 0;
  ...
  const handleWeekClickInternal = (weekIndex) => {
  ...
  const renderHeptagon = (weekIndex, stack) => {
    ...
    return (
      <g
        ...
        onClick={() => handleWeekClickInternal(weekIndex)}
      >
        <defs>
  ...
  allWeeks.forEach((week, index) => {
    if (week.primaryMonth !== lastMonth && week.primaryYear === 2026) {
      monthStarts.push({ index, month: week.monthName, short: week.monthName.slice(0, 3) });
  ...
  return (
    ...
    >
      <div style={{
        ...
        {monthStarts.map(({ index, short }) => (
          <div
            key={index}
            onClick={() => handleWeekClickInternal(index)}
            style={{
              padding: '6px 10px',
              borderRadius: '4px',
              background: allWeeks[activeWeek]?.monthName === allWeeks[index]?.monthName &&
                allWeeks[activeWeek]?.primaryYear === allWeeks[index]?.primaryYear
      ...
      <div style={{
        ...
        {allWeeks.filter((_, i) => i % 4 === 0 || i === activeWeek).map((week) => {
          ...
          return (
            <div
              key={actualIndex}
              onClick={() => handleWeekClickInternal(actualIndex)}
              style={{
